...
bool contains_var(char **expressions, int max_expr, char c){
    for (char **p = expressions; p < expressions + max_expr; p++)
        if (**p == c)
            return true;
    return false;
}
...
int store_expressions(char **expressions, int max_expr, char *expr, char *end, int *num_expr){
    ...
    for (char *p = expr; *p != '\0'; p++){
        if (isalpha(*p) && !contains_var(expressions, max_expr, *p)){
            char var[2] = {*p, '\0'};
            ...
            if (*(p + 1) == '('){
                ...
                if (!contains_expr(expressions, max_expr, p, end)){
                    add_expr(expressions, max_expr, p, end, num_expr);
                }
                p = end - 1;
            } else if (isalpha(*(p + 1) && !contains_expr(expressions, max_expr, p, p + 1))){
    ...
    for (char *p = expr; *p != '\0'; p++){
        ...
        if (*p++ == '&'){ 
            ...
            if (!contains_expr(expressions, max_expr, prev, p))
